...
@Component({
  ...
  template: `
    ...
    @else if (provider(); as person) {
      <section class="section section-tight"><div class="container detail-layout profile-detail-layout">
        <aside class="profile-sidebar">
          <section class="content-card profile-overview-card">
            ...
            <div class="profile-identity-panel">
              ...
              <div class="profile-intro">
                <div class="provider-name-line"><h1 data-cvp-no-localize>{{ person.name }}</h1>@if (person.verified) { <span class="verified" title="Perfil aprovado">✓<span class="sr-only">Perfil aprovado</span></span> }</div>
                <p>{{ person.headline }}</p>
                <span class="profile-location"><span data-cvp-no-localize>{{ person.city }}</span>@if (person.state) {, <span data-cvp-no-localize>{{ person.state }}</span> }</span>
                <cvp-rating [rating]="person.rating" [count]="person.reviewCount" />
              </div>
            </div>
            <dl class="profile-sidebar-facts">
              <div><dt>Serviços concluídos</dt><dd>{{ person.completedJobs }}</dd></div>
              <div><dt>Avaliações</dt><dd>{{ person.reviewCount }}</dd></div>
              <div><dt>Tempo de resposta</dt><dd>{{ person.responseTime }}</dd></div>
            </dl>
            <div class="chip-row profile-summary-tags">@if (person.yearsExperience) { <span class="chip chip-soft">{{ person.yearsExperience }} anos de experiência</span> }@for (quality of person.qualities.slice(0, 3); track quality) { <span class="chip chip-soft">{{ quality }}</span> }</div>
          </section>
          <section class="booking-aside"><span class="eyebrow">Próximo horário</span><strong>{{ person.nextAvailability }}</strong><p>@if (person.priceFromCents > 0) { A partir de <b>{{ person.priceFromCents / 100 | appMoney:'BRL':0 }}</b> } @else { <b>Preço calculado na reserva</b> }</p><button class="btn btn-secondary btn-block" type="button" (click)="startConversation(person.id)">Conversar antes de agendar</button><a class="btn btn-primary btn-block" [routerLink]="['/agendar', defaultServiceId()]" [queryParams]="{ profissional: person.id }">Escolher este profissional</a>@if (conversationError()) { <span class="field-error" role="alert">{{ conversationError() }}</span> }<small>Conversa, valor de referência e detalhes ficam protegidos pela plataforma.</small></section>
        </aside>
        <div class="detail-main" role="region" aria-label="Perfil profissional" tabindex="0">
          <section class="content-card professional-summary-card"><div class="section-heading"><div><span class="eyebrow">Perfil aprovado na plataforma</span><h2>Sobre <span data-cvp-no-localize>{{ person.name.split(' ')[0] }}</span></h2></div><span class="verified-transaction-badge">★ Nota de {{ person.rating | appNumber:2:2 }}</span></div><p>{{ person.bio }}</p></section>
          ...
          @if (person.experiences?.length) { <section class="content-card"><div class="section-heading"><div><span class="eyebrow">Trajetória</span><h2>Experiência profissional</h2></div></div><div class="public-timeline">@for (experience of person.experiences; track experience.id) { <article><span class="timeline-dot"></span><div><strong>{{ experience.role }}</strong><span><span data-cvp-no-localize>{{ experience.company }}</span> · {{ experience.startedAt | appDate:'yyyy' }}–{{ experience.current ? 'atual' : (experience.endedAt | appDate:'yyyy') }}</span>@if (experience.description) { <p>{{ experience.description }}</p> }</div></article> }</div></section> }

          @if (person.courses?.length) { <section class="content-card"><div class="section-heading"><div><span class="eyebrow">Formação</span><h2>Cursos e certificados</h2></div></div><div class="public-credentials">@for (course of person.courses; track course.id) { <article><span aria-hidden="true">✓</span><div><strong>{{ course.title }}</strong><small><span data-cvp-no-localize>{{ course.institution }}</span>@if (course.completedAt) { · {{ course.completedAt | appDate:'yyyy' }} }</small></div>@if (course.certificateUrl) { <a [href]="course.certificateUrl" target="_blank" rel="noopener noreferrer">Certificado ↗</a> }</article> }</div></section> }

          <section class="content-card feedback-section"><div class="section-heading"><div><span class="eyebrow">Reservas concluídas</span><h2>Avaliações verificadas</h2></div><cvp-rating [rating]="person.rating" [count]="person.reviewCount" /></div><p class="feedback-intro">Somente clientes que fecharam negócio pela plataforma podem avaliar com estrelas.</p>
            @if (reviews().length) { <div class="review-list">@for (review of reviews(); track review.id) { <article><div class="review-head"><span class="avatar avatar-sm">{{ review.initials }}</span><div><strong data-cvp-no-localize>{{ review.author }}</strong><span><span class="rating"><span aria-hidden="true">★</span> {{ review.rating }}</span><em class="verified-transaction-badge">Reserva verificada</em></span></div><time [attr.datetime]="review.createdAt">{{ review.createdAt | appDate:'d MMM yyyy' }}</time></div>@if (review.serviceName) { <small class="review-service">{{ review.serviceName }}</small> }<p data-cvp-no-localize>{{ review.comment }}</p>@if (review.providerReply) { <div class="provider-reply"><strong>Resposta do profissional</strong><p>{{ review.providerReply }}</p></div> }@if (auth.user()) { <button class="text-button" type="button" (click)="startReport('review', review.id)">Denunciar avaliação</button> }</article> }</div>@if (reviewPage() < reviewLastPage()) { <div class="feedback-load-more"><button class="btn btn-secondary btn-small" type="button" [disabled]="loadingMoreReviews()" (click)="loadMoreReviews()">{{ loadingMoreReviews() ? 'Carregando…' : 'Ver mais avaliações' }}</button></div> } } @else { <p class="muted">As avaliações de reservas concluídas aparecerão aqui.</p> }
          </section>
...
export class ProviderDetailComponent implements OnInit {
  ...
  constructor() {
    effect(() => {
      ...
      if (person) this.updateSeo(person, this.reviews());
    });
  }
  ...
  private updateSeo(provider: ProviderProfile, reviews: Review[]): void {
    ...
    const person: Record<string, unknown> = {
      ...
      knowsAbout: provider.qualities.map((quality) => this.localization.translate(quality))
    };
      ...
      structuredData: [
        person,
        {
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: this.localization.translate('Profissionais'), item: '/profissionais' },
